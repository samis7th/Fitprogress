export default function MetaProgress({ meta, atual = 0 }) {
  ...
  return (
    <div>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {concluidaManual && (
              <span className="badge-soft px-2.5 py-1 text-xs font-semibold">Concluida</span>
            )}
            {!concluidaManual && atingidaAutomaticamente && (
              <span className="badge-soft px-2.5 py-1 text-xs font-semibold">Atingida</span>
            )}
          </div>

          <p className="app-muted mt-1 text-sm leading-snug">
            {cargaAtual.toFixed(1)} / {metaCarga.toFixed(1)}kg
            {metaRepeticoes ? ` - alvo ${metaRepeticoes} reps` : ""}
          </p>
      ...
      <div className="app-border mt-3 h-2 overflow-hidden rounded-full border bg-black/10">
        <div
